...
# Results
## Hood Canal Stock
The counts and abundance estimates (Table 1, Figure 1) for the Hood Canal stock show variation but no growth from 1991-2019. Without counts in the 1970s and 1980s it was unreasonable to attempt to was drawn about the status of this stock.  Jeffries et al. (2003) used a value from 1978 which was a ground count and Hood Canal was part of the inland stock at that time.  We decided it would be better to use aerial counts consistently. An aerial survey was conducted in the 1970s and 1980s but the required data were not available for this analysis.   The current Nmin value in 2019 for the stock was estimated to be ```r floor(HC$Nmin[nrow(HC)])```.
...
## Growth Curve Model
Growth curves were fitted to the 6 regions within the Southern Puget Sound stock, Northern Inland stock and Washington portion of the WA/OR coastal stock (Figures 2-4). The AICc values for the fitted models with Rmax constant, varying by stock and varying by region were ```r formatC(aic0,digits=4)```, ```r formatC(aic1,digits=4)```, and ```r formatC(aic2,digits=4)```.  The support for any one model was not overwhelming but we chose the stock variation model which had the minimum AICc. The analysis could be expanded to use model averaging but we don't expect this would change any of the conclusions about the stock status. The estimate of $z$ (Table 10) was at the lower bound of 1 which means that $MNPL=K/2$ although there was substantial variation in the estimate of $z$ and all of the estimates including Rmax and $K$. Values of Rmax were all larger than the default PBR value of 0.12 for pinnipeds but it is important to recognize that the confidence intervals did include 0.12 and $z$ and Rmax are negatively correlated in the model and these values are high because $z$ is at its lower bound. Thus, we recommend using the default 0.12 value in PBR calculations for these stocks.
